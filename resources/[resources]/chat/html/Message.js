Vue.component('message', {
  ...
  data() {
    return { bubbleEffect: 'lightSpeedInLeft' };
  },
  computed: {
    textEscaped() {
      ...
      if(this.args[0].match(/\[*\]/g) || this.args[0].match(/>>/g)) {
        this.bubbleEffect = 'fadeInDown';
      } else if(this.template || this.args[0].match(/Faryad|MegaPhone/g)) {
        this.bubbleEffect = 'fadeInLeft'
      } else if (this.args[0].match(/\(\(|\)\)/g) && !this.args[0].includes('mige')) {
        this.bubbleEffect = 'slideInUp'
      } else if (this.templateId == 'weazel' || this.templateId == 'announce') {
        this.bubbleEffect = 'lightSpeedInLeft'
      }
      // fadeInDown
      // fadeInDownBig
      // fadeInLeft
