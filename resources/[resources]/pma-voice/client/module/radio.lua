...
AddEventHandler('esx:setJob', function(info)
	job = info.name
end)
...
function setTalkingOnRadio(plySource, enabled)
	...
	exports.IRV_Status:updateIndicators("radio", radioData)
end
...
function setRadioChannel(channel)
	...
	TriggerEvent("IRV-Status:updateRadioState", {enabled = radioEnabled, channel = radioChannel})
end
...
RegisterCommand('+radioe', function()
	if GetConvarInt('voice_enableRadios', 1) ~= 1 then return end
	...
	if not radioPressed and radioEnabled then
		if radioChannel > 0 then
			...
			radioPressed = true
			playMicClicks(true)
			if not (GetConvarInt('voice_disableVehicleRadioAnim', 0) == 1 and IsPedInAnyVehicle(PlayerPedId(), false)) then
				if not disableRadioAnim then
					if job == 'police' or job == 'sheriff' then
						...
						TaskPlayAnim(PlayerPedId(), "cellphone@str", "cellphone_call_listen_a", 8.0, 2.0, -1, 50, 2.0, 0, 0, 0)
					end
				end
			end
...
RegisterKeyMapping('+radioe', 'Talk over Radio', 'keyboard', 'CAPITAL')
