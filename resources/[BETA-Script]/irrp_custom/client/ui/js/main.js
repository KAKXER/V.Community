...
function listener() {
    window.addEventListener('message', (event) => {
        ...
        if (tempData.type === 'open') {
            ...
            } else if (tempData.what === 'cost') {
                currentUpgradeCost = tempData.cost;
                $('#cash').html('<img style="width: 25px;margin-right: 5px;" src="img/icons/upgrades.png" /><span class="upgradeCost">$' + GetNumberWithCommas(currentUpgradeCost)+' | </span>$' + GetNumberWithCommas(currentCash));
...
document.onkeydown = function(event) {
    if (event.which == 9) {
        ...
        $('#options_container').stop(true, true).animate({height: '210px'}, 300);
...
function createMenu(menuId, data, title, defaultOption, whitelistJobName) {
    ...
    $('#options_container').stop(true, true).animate({height: '0px'}, 300, function() {
        ...
        $('#options_container').stop(true, true).animate({height: '210px'}, 300, function() {
            menuLoading = false;
        });
    });
}
